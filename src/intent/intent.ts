 ...
 */
export function evaluateIntentAndConsistency(
  sceneGraphs: Map<number, SceneNode>,
  existingFindings: Finding[]
): { metrics: CritiqueMetrics; findings: Finding[] } {
  const findings: Finding[] = [...existingFindings];

  const buttonRadiusMap = new Map<string, number>();
  let h1FontSize = 0;
  let h2FontSize = 0;
...
function traverseNodes(node: SceneNode, visitor: (node: SceneNode) => void): void {
  visitor(node);
