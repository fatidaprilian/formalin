...
/**
 * Compares AI proposed file content against developer committed content to extract semantic design token deltas.
 */
export function extractDeltas(aiContent: string, devContent: string): DeltaSignal[] {
  ...
  const mutedColors = ['slate', 'zinc', 'neutral', 'gray', 'stone'];
  ...
  let devMutedCount = 0;
  for (const c of mutedColors) {
    devMutedCount += (devContent.match(new RegExp(c, 'g')) || []).length;
  }
